...
export default function StoryCommentBox({ storyId }) {
  ...
  const { data: comments = [], isLoading } = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await requireSupabase().from("comments").select("*").eq("story_id", storyId).order("created_at", { ascending: false });
  ...
  return <section>
    ...
    {/* Existing comments come first; the entry form is always at the bottom. */}
    {isLoading ? <div className="flex justify-center py-8"><Loader2 className="w-5 h-5 animate-spin text-primary" /></div> : visibleComments.length === 0 ? <p className="text-center text-muted-foreground font-light py-8 text-sm">No comments yet. Be the first to share what you think! ✨</p> : <div className="space-y-4 mb-8">{visibleComments.map((comment) => <CommentItem key={comment.id} comment={comment} allComments={comments} onReply={postComment} isPending={isPending} />)}</div>}
