...
export default function StoryProgressList({ history, readChapters = {} }) {
  ...
  Object.values(history).forEach((entry) => {
    if (!entry?.storyId || !Number.isInteger(Number(entry.chapterNumber))) return;
  ...
  return (
    <div className="space-y-3">
      {readingStories.map((story, i) => {
        ...
        const readCount = storyChapters.filter((chapter) => readChapters[`${story.story_code}:${chapter.chapter_number}`]).length;
        const progress = totalChapters > 0 ? Math.min(100, (readCount / totalChapters) * 100) : 0;
        const lastRead = lastReadByStory[story.story_code];
        const destination = lastRead ? `/story/${story.story_code}/chapter/${lastRead.chapterNumber}` : `/story/${story.story_code}`;
        ...
        return (
          <motion.div key={story.id} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.06 }}>
            <Link to={destination} className="flex items-center gap-4 px-4 py-3 rounded-lg bg-card/40 border border-border/30 hover:border-primary/40 transition-all group">
              {story.cover_image ? <img src={story.cover_image} alt={story.title} width="40" height="64" loading="lazy" className="w-10 h-14 object-cover rounded shrink-0" /> : <div className="w-10 h-14 rounded bg-primary/10 flex items-center justify-center shrink-0"><BookOpen className="w-4 h-4 text-primary/50" /></div>}
              <div className="flex-1 min-w-0">
                <p className="text-sm font-medium truncate group-hover:text-accent transition-colors">{story.title}</p>
                <div className="flex items-center gap-2 mt-1.5">
                  <div className="flex-1 h-1.5 bg-muted rounded-full overflow-hidden" aria-label={`${readCount} of ${totalChapters} chapters read`}><div className="h-full bg-gradient-to-r from-primary to-accent rounded-full transition-all duration-500" style={{ width: `${progress}%` }} /></div>
