...
export default function ChatWidget() {
  ...
  return <div className="fixed bottom-20 left-4 lg:bottom-6 lg:left-6 z-50">
    {open && <div className="mb-3 w-[min(90vw,380px)] max-h-[70vh] flex flex-col overflow-hidden rounded-2xl border border-border/60 bg-card/95 shadow-2xl backdrop-blur-xl">
      ...
      {isBanned ? <div className="border-t border-border/40 p-4 text-center text-sm text-destructive">You are currently unable to use chat.</div> : !rulesAccepted ? <div className="border-t border-border/40"><ChatRulesGate onAccept={acceptRules} /></div> : <form onSubmit={sendMessage} className="border-t border-border/40 p-3 space-y-2">
        <div className="flex gap-2"><input value={name} onChange={(e) => setName(e.target.value)} maxLength={40} placeholder="Choose your username" className="min-w-0 flex-1 rounded-lg border border-border/60 bg-background/70 px-3 py-2 text-xs outline-none focus:border-primary" /><button type="button" onClick={saveName} disabled={identityLoading || !name.trim()} className="rounded-lg border border-border/60 px-3 text-xs disabled:opacity-50">{identityLoading ? <Loader2 className="h-3 w-3 animate-spin" /> : "Save"}</button></div>
        <div className="flex gap-2"><input value={draft} onChange={(e) => setDraft(e.target.value)} maxLength={2000} placeholder="Say something..." className="min-w-0 flex-1 rounded-lg border border-border/60 bg-background/70 px-3 py-2 text-sm outline-none focus:border-primary" /><button disabled={loading || !userId || !draft.trim()} className="rounded-lg bg-primary px-3 text-primary-foreground disabled:opacity-50" aria-label="Send message">{loading ? <Loader2 className="h-4 w-4 animate-spin" /> : <Send className="h-4 w-4" />}</button></div>{error && <p className="text-[11px] text-destructive">{error}</p>}
      </form>}
    </div>}
