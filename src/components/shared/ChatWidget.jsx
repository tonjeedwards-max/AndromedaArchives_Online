...
const ROOM_ID = 1;
const OPEN_EVENT = "andromeda-toggle-chat";
...
export default function ChatWidget() {
  ...
  useEffect(() => {
    ...
    window.addEventListener(OPEN_EVENT, toggleChat);
    return () => window.removeEventListener(OPEN_EVENT, toggleChat);
  }, []);
  ...
  return <div className="fixed bottom-20 left-4 lg:bottom-6 lg:left-6 z-[100]">
    {open && <div className="mb-3 w-[calc(100vw-2rem)] sm:w-[min(90vw,380px)] max-h-[70vh] flex flex-col overflow-hidden rounded-2xl border border-border/60 bg-card/95 shadow-2xl backdrop-blur-xl">
    ...
    <button onClick={() => setOpen((value) => !value)} className="hidden lg:flex h-12 w-12 items-center justify-center rounded-full border border-primary/40 bg-card shadow-lg shadow-primary/20 transition-transform hover:scale-105" aria-label="Open chat" title="Chat"><MessageCircle className="h-5 w-5 text-primary" /></button>
  </div>;
}
