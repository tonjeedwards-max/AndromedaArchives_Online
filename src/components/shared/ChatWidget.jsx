...
export default function ChatWidget() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      if (existing) {
        if (!cancelled) { setUserId(existing.id); setIsBanned(Boolean(existing.is_banned)); if (existing.display_name) setName(existing.display_name); }
        return;
      }
  ...
  return <div className="fixed bottom-20 left-4 lg:bottom-6 lg:left-6 z-[100]">
    {open && <div className="mb-3 w-[min(90vw,380px)] max-h-[70vh] flex flex-col overflow-hidden rounded-2xl border border-border/60 bg-card/95 shadow-2xl backdrop-blur-xl">
      ...
      {!rulesAccepted ? <div className="border-t border-border/40"><ChatRulesGate onAccept={acceptRules} /></div> : isBanned ? <div className="border-t border-border/40 p-4 text-center text-sm text-destructive">You are currently unable to use chat.</div> : <>
