...
export default function FloatingActions() {
  ...
  const storyMap = Object.fromEntries(stories.map((s) => [s.story_code, s]));

  const updates = [
    ...posts.map((p) => ({ type: "blog", date: p.created_date, title: p.title, item: p })),
    ...chapters.map((c) => ({ type: "chapter", date: c.created_date, title: c.title, item: c, story: storyMap[c.story_id] })),
  ].sort((a, b) => new Date(b.date) - new Date(a.date)).slice(0, 8);

  const unseenCount = updates.filter((u) => new Date(u.date).getTime() > lastSeen).length;
        ...
        <div className="relative">
          <button
            onClick={() => {
              if (!panelOpen) {
                const now = Date.now();
                sessionStorage.setItem("lastSeenUpdates", now);
                setLastSeen(now);
              }
              ...
              >
                <div className="flex items-center justify-between px-4 py-3 border-b border-border/40">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="font-heading text-sm font-semibold">Latest Updates</span>
                  </div>
                  ...
                  {updates.length === 0 ? (
                    ...
                    updates.map((u, i) => (
                      <motion.div key={i} initial={{ opacity: 0, x: 10 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: i * 0.04 }}>
                        {u.type === "chapter" && u.story ? (
                          <Link
                            to={`/story/${u.item.story_id}/chapter/${u.item.id}`}
                          ...
                          >
                            ...
                            <div className="flex-1 min-w-0">
                              <p className="text-xs text-primary font-medium">{u.story.title}</p>
                              <p className="text-sm font-medium text-foreground/90 truncate group-hover:text-accent transition-colors">
                                Ch. {u.item.chapter_number}: {u.title}
                              </p>
                              <p className="text-[10px] text-muted-foreground mt-0.5">
                                {u.date ? formatDistanceToNow(new Date(u.date), { addSuffix: true }) : "Recently"}
                              </p>
                            </div>
                          ...
                          >
                            ...
                            <div className="flex-1 min-w-0">
                              <p className="text-xs text-accent font-medium">Blog Post</p>
                              <p className="text-sm font-medium text-foreground/90 truncate group-hover:text-accent transition-colors">
                                {u.title}
                              </p>
                              <p className="text-[10px] text-muted-foreground mt-0.5">
