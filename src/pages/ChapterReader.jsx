...
export default function ChapterReader() {
  ...
  return <>
    <SEO
      title={`${story.title} — Chapter ${chapter.chapter_number}: ${chapter.title}`}
      description={`Read Chapter ${chapter.chapter_number}, “${chapter.title},” from ${story.title} on The Andromeda Archive.`}
      path={`/story/${story.story_code}/chapter/${chapter.chapter_number}`}
      type="article"
      image={story.cover_image}
      breadcrumbs={[{ name: "Home", path: "/" }, { name: "Stories", path: "/stories" }, { name: story.title, path: `/story/${story.story_code}` }, { name: `Chapter ${chapter.chapter_number}`, path: `/story/${story.story_code}/chapter/${chapter.chapter_number}` }]}
      structuredData={{
        "@type": "Chapter",
        name: chapter.title,
        isPartOf: { "@type": "Book", name: story.title, url: `https://andromedaarchiveonline.netlify.app/story/${story.story_code}` },
        position: chapter.chapter_number,
        wordCount: chapter.word_count ? Number(chapter.word_count) : undefined,
        url: `https://andromedaarchiveonline.netlify.app/story/${story.story_code}/chapter/${chapter.chapter_number}`,
      }}
    />
