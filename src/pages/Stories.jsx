...
export default function Stories() {
  ...
  const paginatedStories = filtered.slice((clampedPage - 1) * itemsPerPage, clampedPage * itemsPerPage);

  return (
    <div className="relative z-10">
      <div className="pt-20 pb-4 max-w-7xl mx-auto px-4 sm:px-6">
        <SectionHeading title="Story Catalogue" subtitle="Choose your next adventure among the stars" />
      </div>
        ...
        <div className="flex flex-col md:flex-row gap-6 lg:gap-8">
          <aside className="md:w-60 lg:w-64 shrink-0">
            <div className="md:sticky md:top-20 bg-card/40 backdrop-blur-sm border border-border/30 rounded-xl p-4">
              ...
              <StoryFilter searchQuery={searchQuery} onSearchChange={(v) => { setSearchQuery(v); setCurrentPage(1); }} selectedFilters={selectedFilters} onToggleFilter={toggleFilter} selectedStatus={selectedStatus} onToggleStatus={toggleStatus} onClear={() => { setSelectedFilters([]); setSelectedStatus([]); setCurrentPage(1); }} />
            </div>
          </aside>
          <div className="flex-1 min-w-0">
            {isLoading ? (
              ...
              <div className="text-center py-20"><p className="text-muted-foreground font-light text-lg">No stories found. Try adjusting your search or filters.</p></div>
            ) : (
              <>
                <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 sm:gap-6">
                  {paginatedStories.map((story, i) => <StoryCoverCard key={story.id} story={story} index={i} />)}
                </div>
                {totalPages > 1 && (
                  <div className="flex items-center justify-center gap-4 mt-10">
                    <button onClick={() => setCurrentPage((p) => Math.max(1, p - 1))} disabled={clampedPage === 1} className="flex items-center gap-1 text-sm px-4 py-2 rounded-lg border border-border hover:border-primary/40 hover:bg-primary/5 disabled:opacity-30 disabled:cursor-not-allowed transition-all"><ChevronLeft className="w-4 h-4" /> Back</button>
                    <span className="text-sm text-muted-foreground font-medium">{clampedPage} / {totalPages}</span>
                    <button onClick={() => setCurrentPage((p) => Math.min(totalPages, p + 1))} disabled={clampedPage === totalPages} className="flex items-center gap-1 text-sm px-4 py-2 rounded-lg border border-border hover:border-primary/40 hover:bg-primary/5 disabled:opacity-30 disabled:cursor-not-allowed transition-all">Next <ChevronRight className="w-4 h-4" /></button>
                  </div>
                )}
              </>
            )}
