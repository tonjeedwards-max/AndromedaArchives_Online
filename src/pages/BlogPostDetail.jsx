...
export default function BlogPostDetail() {
  ...
  const description = post.excerpt || `Read ${post.title} on The Andromeda Archive.`;
  ...
  return <>
    <SEO
      ...
      path={`/blog/${blogId}`}
      type="article"
      breadcrumbs={[{ name: "Home", path: "/" }, { name: "Blog", path: "/blog" }, { name: post.title, path: `/blog/${blogId}` }]}
      structuredData={{
        "@type": "BlogPosting",
        headline: post.title,
        description,
        datePublished: displayDate ? new Date(displayDate).toISOString() : undefined,
        dateModified: post.updated_at ? new Date(post.updated_at).toISOString() : undefined,
        mainEntityOfPage: { "@type": "WebPage", "@id": `https://andromedaarchiveonline.netlify.app/blog/${blogId}` },
        publisher: { "@type": "Organization", name: "The Andromeda Archive", url: "https://andromedaarchiveonline.netlify.app" },
        keywords: tags.join(", "),
      }}
    />
