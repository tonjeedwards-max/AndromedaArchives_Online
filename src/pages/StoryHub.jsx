  ...
  const { data: chapters = [], isLoading: loadingChapters, error: chaptersError } = useQuery({
    ...
    queryFn: async () => {
      ...
      const { data, error } = await supabase
        .from("chapters")
        .select("id, story_id, chapter_number, title, word_count, published, created_at, updated_at, content, media")
      ...
      <div className="max-w-5xl mx-auto px-4 sm:px-6 py-8">
        <Tabs defaultValue="chapters" className="w-full">
          ...
          <TabsContent value="chapters">
            {loadingChapters ? <div className="flex justify-center py-10"><Loader2 className="w-5 h-5 animate-spin text-primary" /></div> : chaptersError ? <div className="rounded-lg border border-destructive/30 bg-destructive/5 p-5 text-sm text-destructive">We couldn't load the chapters right now. Please refresh and try again.</div> : <ChapterList chapters={chapters} storyCode={story.story_code} />}
          </TabsContent>
